...
docker run -e NODE_ENV=production -e CORE_HOST=https://chat.twake.app -p 80:3123 --name twake-mobile-server-web-qa -d twake-mobile-server
docker run -e NODE_ENV=production -e CORE_HOST=https://web.qa.twake.app -p 3129:3123 --name twake-mobile-server-chat -d twake-mobile-server
